...
- Added a recovery-safe legacy `pharmacy.patient.mobile` compatibility alias that reads and writes Odoo's native `res.partner.phone` value.
- Added the matching hidden prescription compatibility alias so databases interrupted during the previous upgrade can initialize before their views are refreshed.
- The patient and prescription interfaces continue to display only Odoo's native Phone field.

## 19.0.4.0.1

- Removed the invalid `res.partner.mobile` dependency that prevented Odoo 19 Community from initializing its model registry.
- Consolidated patient contact details and partial-fill follow-up on Odoo's native `res.partner.phone` field.
