...
class TestNoelsPharmacyCore(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.expiry_record = cls.env["pharmacy.expiry.record"].create(
            {
                "product_id": cls.product.id,
                "expiry_date": fields.Date.context_today(cls.env.user) + relativedelta(years=1),
                ...
                "line_ids": [
                    (
                        ...
                        0,
                        {
                            ...
                            "expiry_record_id": self.expiry_record.id,
                        },
                    )
                ],
            }
    ...
    def test_partial_fill_enters_follow_up_queue(self):
        prescription = self._create_prescription()
        prescription.write({"refills_allowed": 0})
        prescription.action_submit()
        prescription.action_approve()
        dispensing = prescription._get_or_create_prepared_dispensing()
        dispensing.line_ids.quantity = 7
        dispensing.action_confirm()

        self.assertEqual(prescription.state, "partially_dispensed")
        self.assertTrue(prescription.partial_fill_open)
        self.assertEqual(prescription.partial_fill_remaining_qty, 14)
        self.assertEqual(prescription.follow_up_status, "pending")
        self.assertTrue(prescription.follow_up_date)

    def test_patient_contact_fields_use_linked_contact(self):
        self.patient.write(
            {
                "phone": "868-555-0100",
                "emergency_contact_name": "Jane Mohammed",
                "emergency_contact_phone": "868-555-0101",
            }
        )
        self.assertEqual(self.partner.phone, "868-555-0100")
        self.assertEqual(self.patient.emergency_contact_name, "Jane Mohammed")
