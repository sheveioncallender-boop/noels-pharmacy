...
class PharmacyRegisterType(models.Model):
    ...
    _sql_constraints = [
        ("code_company_unique", "unique(code, company_id)", "The register code must be unique per company."),
    ]
...
class PharmacyActiveIngredient(models.Model):
    ...
    _sql_constraints = [
        ("ingredient_name_unique", "unique(name)", "The active ingredient already exists."),
    ]
