...
class PharmacyPrescription(models.Model):
    ...
    _sql_constraints = [
        ("refills_nonnegative", "check(refills_allowed >= 0)", "Refills allowed cannot be negative."),
    ]
    ...
    def action_print_labels(self):
        ...
        if not self.line_ids:
            raise UserError(_("There are no medication lines to print."))
        return self.env.ref("noels_pharmacy.action_report_pharmacy_label").report_action(self.line_ids)
