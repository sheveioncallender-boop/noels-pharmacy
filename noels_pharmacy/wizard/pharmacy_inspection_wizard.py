...
class PharmacyInspectionWizard(models.TransientModel):
    ...
    def action_export_csv_pack(self):
        ...
        with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as archive:
            ...
            partial_fills = prescriptions.filtered("partial_fill_open")
            self._write_csv(
                archive,
                "partial_fill_follow_up.csv",
                ["RX Number", "Patient", "Phone", "Email", "Outstanding Medicines", "Partial Balance", "Last Dispensed", "Follow-up Due", "Follow-up Status", "Contact Count", "Last Contact", "Last Contacted By", "Notes"],
                [
                    [
                        rx.name,
                        rx.patient_id.name,
                        rx.patient_mobile or rx.patient_phone or "",
                        rx.patient_email or "",
                        rx.outstanding_medicines or "",
                        rx.partial_fill_remaining_qty,
                        rx.last_dispensed_at or "",
                        rx.follow_up_date or "",
                        rx.follow_up_status,
                        rx.follow_up_count,
                        rx.last_follow_up_at or "",
                        rx.last_follow_up_by_id.name or "",
                        rx.follow_up_notes or "",
                    ]
                    for rx in partial_fills
                ],
            )
            ...
            self._write_csv(
                archive,
                "dispensing_log.csv",
                ["Dispensing", "RX Number", "Date Dispensed", "Patient", "Medicine", "Quantity Supplied", "UoM", "Remaining Balance", "Pharmacist Initials", "Pharmacy Expiry", "Lot/Batch", "Source", "State"],
                [
                    [
            ...
            for register_type in self._register_types():
                ...
                self._write_csv(
                    archive,
                    filename,
                    ["Date/Time", "Movement", "RX Number", "Patient", "Doctor", "Medicine", "Pharmacy Expiry", "Lot/Batch", "Qty In", "Qty Out", "UoM", "Pharmacist Initials", "Source"],
                    [
                        [
            ...
            expiry_records = self.env["pharmacy.expiry.record"].search(
                [
                    ("company_id", "=", self.company_id.id),
                    ("active", "=", True),
                ],
                order="expiry_date, product_id, id",
            )
            self._write_csv(
                archive,
                "expiry_monitoring.csv",
                ["Reference", "Internal Reference", "Medicine", "Expiry Date", "Days to Expiry", "Status", "Received Qty", "Allocated Qty", "On Hand", "Receipt", "Vendor"],
                [
                    [
                        record.name,
                        record.internal_reference or "",
                        ...
                        record.on_hand_qty,
                        record.stock_picking_id.name or "",
                        record.vendor_id.name or "",
                    ]
                    for record in expiry_records
                ],
            )
            medicines = self.env["product.template"].with_company(self.company_id).search(
                [("is_pharmacy_item", "=", True)],
                order="name, id",
            )
            self._write_csv(
                archive,
                "low_stock_monitoring.csv",
                ["Internal Reference", "Medicine", "On Hand", "Forecasted", "Low Stock Threshold", "Status"],
                [
                    [
                        medicine.default_code or "",
                        medicine.display_name,
                        medicine.qty_available,
                        medicine.virtual_available,
                        medicine.pharmacy_low_stock_threshold,
                        medicine.pharmacy_low_stock_status,
                    ]
